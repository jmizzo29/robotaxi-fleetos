...
export default async function handler(req, res) {
  ...
  if (!hasPostgres()) {
    res.status(503).json({ error: 'DATABASE_REQUIRED', message: 'Postgres is required for FleetOS accounts.' });
    return;
  }
  ...
  if (!validateInviteCode(req.body?.inviteCode)) {
    res.status(403).json({
      ...
      message: 'FleetOS beta is invite-only. Enter the invite code provided to this tester.',
    });
    return;
  }
