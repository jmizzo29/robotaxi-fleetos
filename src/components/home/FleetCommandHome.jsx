...
const CARD_PAD = 'p-4';
const SECTION_PRIMARY = 'mt-7';
const SECTION_SECONDARY = 'mt-6';
const SECTION_TERTIARY = 'mt-5';

function SectionHead({ title, actionLabel, onAction, tier = 'secondary' }) {
  ...
  return (
    <div className={`flex items-center justify-between gap-3 ${tier === 'primary' ? 'mb-3.5' : 'mb-3'}`}>
...
function EarningsHeroCard({ hero }) {
  return (
    <section aria-label="Net earnings today">
      <div
        className="relative overflow-hidden rounded-[24px] border border-white/25 px-5 pb-5 pt-4 shadow-[0_24px_56px_-24px_rgba(37,99,235,0.72)]"
      ...
      >
        ...
        <div className="relative flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 text-[4rem] font-bold leading-[0.9] tracking-[-0.045em] text-white">
              {hero.amount}
            </p>
            {hero.delta ? (
        ...
        <div className="relative mt-4 grid grid-cols-3 gap-2 border-t border-white/25 pt-4">
          <div className="text-center">
            ...
            <p className="mt-1.5 text-[11px] font-semibold uppercase tracking-[0.06em] text-white/70">Trips</p>
          </div>
          <div className="border-x border-white/20 text-center">
            ...
            <p className="mt-1.5 text-[11px] font-semibold uppercase tracking-[0.06em] text-white/70">Tesla Share</p>
          </div>
          <div className="text-center">
            <p className="text-[28px] font-bold leading-none text-white">{hero.netMargin || '—'}</p>
            <p className="mt-1.5 text-[11px] font-semibold uppercase tracking-[0.06em] text-white/70">Net Margin</p>
          </div>
        </div>
      </div>
    </section>
...
function FleetStatusGrid({ strip, onNavigate }) {
  ...
  return (
    <section className={SECTION_TERTIARY} aria-label="Fleet status">
      <SectionHead title="Fleet Status" actionLabel="View all" onAction={() => onNavigate('fleet')} tier="secondary" />
        ...
        {cards.map((card) => {
          ...
          return (
            <button
              ...
              className={`${CARD} ${CARD_PAD} text-left transition active:scale-[0.99]`}
            >
              ...
              <p className={`mt-2.5 text-[38px] font-bold leading-none tabular-nums ${card.accent}`}>{card.value}</p>
  ...
  if (eventType === 'trip' || eventType === 'milestone') {
    return (
      <span className={`${shell} bg-[#ecfdf3] text-[#15803d]`}>
        <CircleDollarSign className="h-[18px] w-[18px]" strokeWidth={2.2} />
      </span>
    );
  }
  if (eventType === 'surge') {
...
function FleetActivitySection({ events }) {
  return (
    <section className={SECTION_SECONDARY} aria-label="Fleet activity">
      <SectionHead title="Fleet Activity" tier="secondary" />
      <ul className="space-y-2.5">
        {events.map((event) => (
          <li
            key={event.id}
            className={`flex items-center gap-3 ${CARD} px-3.5 py-3.5 shadow-[0_4px_18px_-16px_rgba(15,23,42,0.22)]`}
          >
...
function AiOperationsBrief({ plan, onNavigate }) {
  return (
    <section className={`${SECTION_TERTIARY} pb-2`} aria-label="AI Operations Brief">
      <SectionHead title="AI Operations Brief" actionLabel="Execute" onAction={() => onNavigate('ai')} tier="tertiary" />
      <div className={`${CARD} border-l-[4px] border-l-[#2563eb] ${CARD_PAD} shadow-[0_4px_20px_-18px_rgba(15,23,42,0.2)]`}>
        <div className="flex items-center gap-2">
          <ClipboardList className="h-4 w-4 text-[#2563eb]" strokeWidth={2.2} />
          <p className="text-[11px] font-bold uppercase tracking-[0.1em] text-slate-500">Prepared for your fleet</p>
        </div>
        <p className="mt-3 text-[18px] font-bold leading-snug text-slate-950">{plan.summary}</p>
        <p className="mt-2 text-[14px] font-semibold text-[#2563eb]">{plan.action}</p>
        <div className="mt-4 grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-slate-400">Expected demand increase</p>
            <p className="mt-1.5 text-[24px] font-bold tabular-nums text-[#2563eb]">{plan.demandIncrease}</p>
          </div>
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-slate-400">Est. additional earnings</p>
            <p className="mt-1.5 text-[24px] font-bold tabular-nums text-[#15803d]">{plan.expectedRevenueImpact}</p>
          </div>
        </div>
